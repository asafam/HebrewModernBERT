#!/bin/bash -i
# HebrewModernBERT-LARGE phase-1: 8192 context, sequence-packed (per-document cu_seqlens, so no
...
#
# Submit with: sbatch .slurm/jobs/train_b200_large_phase-1.slurm
...
export TRITON_CACHE_DIR="$HOME/.cache/hmb/triton-large-phase1"
export TORCHINDUCTOR_CACHE_DIR="$HOME/.cache/hmb/inductor-large-phase1"
mkdir -p "$TRITON_CACHE_DIR" "$TORCHINDUCTOR_CACHE_DIR"

RUN_NAME="${RUN_NAME:-modern-bert-large-phase-1-contextextension}"
...
echo "Starting composer: yamls/main/base_hebrew_large/flex-bert-rope-phase-1-contextextension.yaml"
